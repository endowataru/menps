...
#include <menps/meult/sm/impl/ult_desc_pool.hpp>
...
{
    const mefdn::size_t stack_size = 2048 * 1024; // TODO
    
    const auto sp_start = new mefdn::uint8_t[stack_size];
    ...
    MEFDN_LOG_VERBOSE(
        ...
        "stack_ptr:{:x}\t"
        "stack_size:{}"
    ,   reinterpret_cast<mefdn::uintptr_t>(desc)
    ,   reinterpret_cast<mefdn::uintptr_t>(desc->stack_ptr)
    ,   desc->stack_size
    );
...
void ult_desc_pool::destory_ult(ult_ptr_ref&& th)
{
    ...
    const auto sp_end = static_cast<mefdn::uint8_t*>(desc->stack_ptr);
    ...
    ,   reinterpret_cast<mefdn::uintptr_t>(desc->stack_ptr)
    ,   desc->stack_size
    ,   reinterpret_cast<mefdn::uintptr_t>(sp_start)
    );
