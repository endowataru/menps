...
void queue_pair_deleter::operator () (ibv_qp* const qp) const noexcept
{
    if (qp == nullptr) {
...
{
    static const mefdn::uint32_t default_max_send_wr = 1 << 14; // TODO: magic number
    static const mefdn::uint32_t default_max_recv_wr = 1 << 14; // TODO: magic number
    static const mefdn::uint32_t default_max_send_sge = 1; // Scatter/Gather
    static const mefdn::uint32_t default_max_recv_sge = 1;
    ...
    attr.send_cq             = nullptr; // Filled by users
    attr.recv_cq             = nullptr; // Filled by users
    attr.srq                 = nullptr;
    ...
    attr.pd                  = nullptr; // Set by make_queue_pair later
...
qp_init_attr_t make_default_rc_qp_init_attr(const device_attr_t& dev_attr)
{
    ...
    #ifdef MEDEV_IBV_MASKED_ATOMICS_SUPPORTED
...
qp_attr_t make_default_qp_attr(const device_attr_t& dev_attr)
{
    ...
    qp_attr.max_rd_atomic =
        static_cast<mefdn::uint8_t>(
            std::min(
                dev_attr.max_qp_rd_atom
            ,   static_cast<int>(std::numeric_limits<mefdn::uint8_t>::max())
            )
        );
...
inline void modify_qp_init_to_rtr(
    ibv_qp* const           qp
,   const mefdn::uint32_t  qp_num
,   const mefdn::uint16_t  lid
,   const ibv_device_attr&  device_attr
    ...
    attr.rq_psn                = 0; // PSN starts from 0
    attr.max_dest_rd_atomic    = static_cast<mefdn::uint8_t>(max_dest_rd_atomic);
