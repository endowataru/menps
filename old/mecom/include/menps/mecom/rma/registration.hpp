...
{
    void* const ptr = new mefdn::uint8_t[size_in_bytes];
...
inline void deallocate_region(const local_region& region)
{
    ...
    delete[] static_cast<mefdn::uint8_t*>(to_raw_pointer(region));
}
...
,   const local_ptr<T>& ptr
    ...
    return remote_ptr<T>::cast_from(
        mecom::rma::untyped::use_remote_address(reg, proc_id, ptr.to_address())
    );
}
template <typename T>
inline remote_ptr<T> use_remote_ptr(
