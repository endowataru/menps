...
public:
    ...
    virtual untyped::local_region register_region(const untyped::register_region_params& params) MEFDN_OVERRIDE
    {
        ...
        const untyped::local_region reg = { key, reinterpret_cast<mefdn::uint64_t>(mr_ptr) };
        return reg;
    }
...
mefdn::unique_ptr<rma::registrator> make_rma_registrator(endpoint& ep)
{
    return mefdn::make_unique<rma::ibv_registrator>(ep);
}
