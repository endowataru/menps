...
private:
    ...
    rpc::requester& get_rpc_requester() const noexcept {
        return rqstr_;
    }
    mpi_interface& get_mpi_interface() const noexcept {
        return mi_;
    }
...
mefdn::unique_ptr<rma::requester> make_rma_requester(rpc::requester& req, mpi_interface& mi)
{
    return mefdn::make_unique<mpi_requester>(req, mi);
}
