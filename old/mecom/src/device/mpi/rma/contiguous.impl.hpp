...
protected:
    void setup()
    {
        ...
        comm_ = mi.comm_dup(MPI_COMM_WORLD, "MECOM_COMM_RMA_EMULATOR");
...
private:
    struct read_handler
    {
        ...
        typename ServerCtx::return_type operator() (ServerCtx& sc) const
        {
            ...
            ,   src_proc
            ,   reinterpret_cast<mefdn::intptr_t>(req.src_ptr)
            ,   req.size_in_bytes
            ,   req.tag
            );
...
public:
    MEFDN_NODISCARD
    virtual ult::async_status<void> async_read(const rma::untyped::read_params& params) MEFDN_OVERRIDE
    {
        auto& self = this->derived();
        auto& mi = self.get_mpi_interface();
        ...
        ,   params.src_proc
        ,   static_cast<mefdn::intptr_t>(rma::to_integer(params.src_raddr))
        ,   static_cast<mefdn::intptr_t>(rma::to_integer(params.dest_laddr))
    ...
    struct write_handler
    {
        ...
        typename ServerCtx::return_type operator() (ServerCtx& sc) const
        {
            ...
            MEFDN_LOG_DEBUG(
                ...
                "src_proc:{}\taddr:{:x}\tsize_in_bytes:{}\ttag:{}"
            ,   src_proc
            ,   reinterpret_cast<mefdn::intptr_t>(req.dest_ptr)
            ,   req.size_in_bytes
            ,   req.tag
            );
...
public:
    MEFDN_NODISCARD
    virtual ult::async_status<void> async_write(const rma::untyped::write_params& params) MEFDN_OVERRIDE
    {
        auto& self = this->derived();
        auto& mi = self.get_mpi_interface();
        ...
        ,   params.dest_proc
        ,   static_cast<mefdn::intptr_t>(rma::to_integer(params.dest_raddr))
        ,   static_cast<mefdn::intptr_t>(rma::to_integer(params.src_laddr))
        ,   params.size_in_bytes
        ,   tag
        );
...
private:
    ...
    int new_tag()
    {
        ...
        const int tag = tag_.fetch_add(1, mefdn::memory_order_relaxed);
        return tag;
    }

    mefdn::atomic<int> tag_;
    MPI_Comm            comm_;
