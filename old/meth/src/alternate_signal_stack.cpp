...
{
public:
    impl(void* const stack_first_ptr, const mefdn::size_t size_in_bytes)
    {
        ...
        if (ret == 0) {
            MEFDN_LOG_DEBUG(
                ...
                "stack_size:{}\t"
                "ret:{}"
            ,   reinterpret_cast<mefdn::uintptr_t>(stack_first_ptr)
            ,   size_in_bytes
            ,   ret
            );
        }
...
alternate_signal_stack::alternate_signal_stack(void* const stack_first_ptr, const mefdn::size_t size_in_bytes)
