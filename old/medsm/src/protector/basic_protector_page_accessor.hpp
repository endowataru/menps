...
public:
    ...
    void do_for_all_blocks_in(const mefdn::uintptr_t idx_in_pg, const mefdn::size_t size_in_bytes, Func func)
    {
    ...
    template <typename Func>
    void do_for_block_at(const mefdn::uintptr_t idx_in_pg, Func&& func)
    {
        auto& self = this->derived();
        ...
        mefdn::forward<Func>(func)(blk_ac);
    }
...
private:
    struct block_pos
    {
        ...
        mefdn::uintptr_t   idx_in_blk;
    };
    
    block_pos get_block_pos(const mefdn::uintptr_t idx_in_pg) const
    {
        auto& self = this->derived();
