...
public:
    ...
    void do_for_all_blocks_in(const mefdn::uintptr_t idx_in_seg, const mefdn::size_t size_in_bytes, Func func)
    {
    ...
    do_for_block_at(const mefdn::uintptr_t idx_in_seg, Func&& func)
    {
        ...
        return pg_ac.do_for_block_at(pg_info.idx_in_pg, mefdn::forward<Func>(func));
    }
    
private:
    struct page_pos {
        page_id_type        pg_id;
        mefdn::uintptr_t   idx_in_pg;
    };
    
    page_pos get_page_pos(const mefdn::uintptr_t idx_in_seg) const
    {
        auto& self = this->derived();
