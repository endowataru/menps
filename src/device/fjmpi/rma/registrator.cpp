    ...
    virtual untyped::local_region register_region(const untyped::register_region_params& params) MGBASE_OVERRIDE
    {
        ...
        return untyped::make_local_region(
            untyped::make_region_key(
                params.local_ptr
            ,   static_cast<mgbase::uint64_t>(res.memid)
            )
        ,   res.laddr
        );
    }
...
mgbase::unique_ptr<rma::registrator> make_rma_registrator(fjmpi_interface& fi)
{
    return mgbase::make_unique<rma::fjmpi_registrator>(fi);
}
