...
struct call_roundtrip_request_info {
    call_roundtrip_cb*                  cb;
    typename Handler::reply_argument*   dest;
    typename Handler::request_argument  arg;
};

template <typename Handler>
struct call_roundtrip_reply_info {
    call_roundtrip_cb*                  cb;
    typename Handler::reply_argument*   dest;
    typename Handler::reply_argument    arg;
};
...
public:
    ...
    )
    {
        ...
        detail::call_roundtrip_request_info<Handler> info = { &cb, result, arg };
...
template <typename Handler>
inline mgbase::deferred<void> call_roundtrip_nb(
    call_roundtrip_cb&                  cb
,   process_id_t                        proc_id
,   typename Handler::request_argument& arg
,   typename Handler::reply_argument*   result
...
{
    typedef detail::call_roundtrip_request_info<Handler>    request_info;
    typedef detail::call_roundtrip_reply_info<Handler>      reply_info;
    ...
    struct transfer
    {
        static void request(const callback_parameters* params)
        {
            ...
            const reply_info rep_info = { req_info.cb, req_info.dest, result };
