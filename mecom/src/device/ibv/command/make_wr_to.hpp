...
,   atomic_buffer&          //atomic_buf
    ...
    wr->send_flags            =
        (cmd.size_in_bytes < MECOM_IBV_SEND_INLINE_SIZE) ? IBV_SEND_INLINE : 0;
    
    wr->wr.rdma.remote_addr   = cmd.raddr;
    wr->wr.rdma.rkey          = cmd.rkey;
    
    sge->addr   = cmd.laddr;
    sge->length = static_cast<mefdn::uint32_t>(cmd.size_in_bytes);
    sge->lkey   = cmd.lkey;
...
,   atomic_buffer&              atomic_buf
    ...
    sge->length = sizeof(mefdn::uint64_t);
...
,   atomic_buffer&              atomic_buf
    ...
    *r.buf_lptr = cmd.value;
    ...
    wr->opcode                = IBV_WR_RDMA_WRITE;
    wr->send_flags            =
        (sizeof(mefdn::uint64_t) < MECOM_IBV_SEND_INLINE_SIZE) ? IBV_SEND_INLINE : 0;
