...
#define MGCOM_FJMPI_COMMAND_EXECUTE_CASES(CASE, RETURN, params, completer) \
    CASE(fjmpi_get) { \
        MGBASE_STATIC_ASSERT(sizeof(get_params) <= sizeof(params)); \
        const bool ret = ::mgcom::fjmpi::try_execute_get(reinterpret_cast<const get_params&>(params), (completer)); \
        RETURN(ret) \
    } \
    CASE(fjmpi_put) { \
        MGBASE_STATIC_ASSERT(sizeof(put_params) <= sizeof(params)); \
        const bool ret = ::mgcom::fjmpi::try_execute_put(reinterpret_cast<const put_params&>(params), (completer)); \
        RETURN(ret) \
    }
